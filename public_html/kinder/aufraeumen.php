...
<html lang="de">
	<head>
		...
		<style>
			...
			.muell {
				position: absolute; /* Quelle: https://developer.mozilla.org/en-US/docs/Web/CSS/position */
				width: 60px;
				cursor: grab; /* Quelle: https://developer.mozilla.org/en-US/docs/Web/CSS/cursor */
				z-index: 100;
			}
			.muell.dragging {
				cursor: grabbing;
			...
			#sprechblase {
				...
				backdrop-filter: blur(12px); /* Quelle: https://developer.mozilla.org/en-US/docs/Web/CSS/backdrop-filter */
				padding: 20px 24px;
				border-radius: 20px;
				width: 360px;
				height: 160px;
	...
	<body class="level-page">
			...
			<div id="sprechblase-container">
				<div id="sprechblase">
					...
					<div class="level-buttons" id="abschluss-buttons" style="display: none;">
						...
						<a href="fuchsbau.php" class="zurueck-btn" onclick="window.location.reload(); return false;">Nochmal spielen</a> <!-- Quelle: https://developer.mozilla.org/en-US/docs/Web/API/Location/reload -->
					</div>
				</div>
			</div>
		</div>
		...
		<script>
			...
			const texte = [
				...
				"Du kannst den Wald jetzt noch genauer erkunden.",
				"In 8 Leveln zeig ich dir wie sehr alles im Wald zusammenhängt.",
				"Willst du gleich weiter?"
			];
			let currentText = 0;
			let levelAbgeschlossen = false;
			function updateText() {
				if (!levelAbgeschlossen) { // Noch nicht abgeschlossen → Pfeile sperren
			...
			document.addEventListener('mousemove', e => { 
				...
				} // Quelle:  https://developer.mozilla.org/en-US/docs/Web/API/Element/mousemove_event // https://developer.mozilla.org/en-US/docs/Web/API/Event/preventDefault // https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent/clientX
			});
			...
			document.addEventListener('mouseup', e => {
				if (dragging) {
					...
					const muellRect = muell.getBoundingClientRect(); // Quelle: https://developer.mozilla.org/en-US/docs/Web/API/Element/getBoundingClientRect
					const eimerRect = eimerBereich.getBoundingClientRect();
					const overlaps = !( // Bounding Box Kollision 
